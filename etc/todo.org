...
** TODO [#B] shutdown the 'ping' and 'recovery' threads
** TODO [#A] implement failure detection heartbeat thread
** TODO [#C] use conditionals in waitForVictor 
** TODO [#C] cleanup the failure detector, index to id & id to index
** TODO [#A] in consensus manager drop message not originating from current leader
...
** TODO [#B] implement received message dropping from dead nodes
...
** TODO [#A] make the client refresh the looked up port when replication timesout
* TODO doc
